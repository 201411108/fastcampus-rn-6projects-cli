...
export function useStepCounterBackgroundSync({
  ...
  const syncIosSessionStepCount = useCallback(async (sessionStartDate: Date) => {
    ...
    const syncedStepCount = await new Promise<number>((resolve, reject) => {
      queryStepCounterDataBetweenDates(sessionStartDate, now, (error, data) => {
        if (error) {
