...
const STEP_INSIGHT_TIMEOUT_MS = 10_000;
...
function withTimeout<T>(task: Promise<T>, timeoutMs: number): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    const timeoutId = setTimeout(() => {
      reject(new Error('AI 요청 시간이 초과되었습니다.'));
    }, timeoutMs);
...
export async function generateStepInsightWithAi(
  ...
  try {
    const ai = getAI();
    const model = getGenerativeModel(ai, { model: STEP_INSIGHT_MODEL });
    const result = await withTimeout(
      model.generateContent(buildPrompt({ stepCount, goalStepCount, progressPercent })),
      STEP_INSIGHT_TIMEOUT_MS,
    );
    const responseText = result.response.text();
    return {
      data: parseInsightResponse(responseText),
      isFallback: false,
    };
  } catch {
    return {
...
export function getEmptyStepInsightResult() {
  return EMPTY_STEP_INSIGHT_RESULT;
}
