...
const stepInsightStorage = createAsyncStorage('stepInsightHistory');
...
export async function getStepInsightHistory(): Promise<StepInsightHistoryItem[]> {
  try {
    const rawHistory = await stepInsightStorage.getItem(STEP_INSIGHT_HISTORY_STORAGE_KEY);
...
export async function appendStepInsightHistory(item: StepInsightHistoryItem) {
  try {
    ...
    await stepInsightStorage.setItem(
      STEP_INSIGHT_HISTORY_STORAGE_KEY,
...
export async function clearStepInsightHistory() {
  try {
    await stepInsightStorage.removeItem(STEP_INSIGHT_HISTORY_STORAGE_KEY);
  } catch (error) {
