...
import type {WeeklyReportDataSources, WeeklyReportDayPoint} from '@rn-health/feature-daily-report';
...
async function loadWeekSeries(mondayISO: string): Promise<WeeklyReportDayPoint[]> {
  ...
  const stepByDate = buildStepSummaryMap(rawStep, dates);

  return dates.map(date => {
    const records = allFood.filter(record => isSameISODate(record.createdAt, date));
    return {
      date,
